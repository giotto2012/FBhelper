...
- (BOOL) setConstraintConstant:(CGFloat)constant forAttribute:(NSLayoutAttribute)attribute
{
    ...
    {
        [self.superview addConstraint: [NSLayoutConstraint constraintWithItem:self attribute:attribute relatedBy:NSLayoutRelationEqual toItem:self.superview attribute:attribute multiplier:1.0f constant:constant]];
        return NO;
    }
}
- (BOOL) setSecondConstraintConstant:(CGFloat)constant forAttribute:(NSLayoutAttribute)attribute

{
    NSLayoutConstraint * constraint = [self secondConstraintForAttribute:attribute];
    if(constraint)
    {
        [constraint setConstant:constant];
        return YES;
...
- (NSLayoutConstraint*) constraintForAttribute:(NSLayoutAttribute)attribute
{
    ...
    if (attribute == NSLayoutAttributeWidth || attribute == NSLayoutAttributeHeight)
    {
        ...
        fillteredArray = [[self.superview constraints] filteredArrayUsingPredicate:predicate];
    ...
    }else
    {
        ...
        return fillteredArray.lastObject;
    }
}

- (NSLayoutConstraint*) secondConstraintForAttribute:(NSLayoutAttribute)attribute
{
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"secondAttribute = %d && secondItem = %@", attribute, self];
